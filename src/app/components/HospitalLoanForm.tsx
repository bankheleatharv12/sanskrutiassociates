...
export function HospitalLoanForm() {
  ...
  const buildWhatsAppMessage = () => {
    const appId = applicationId || `HOSP-${Date.now().toString().slice(-8)}`;
    const customerTypeName = customerTypes.find(t => t.id === customerType)?.name || customerType;
    
    let message = `*SANSKRUTI ASSOCIATES*\n*New Loan Application*\n\n━━━━━━━━━━━━━━━━━━━━\n\n● Application ID: *${appId}*\n● Loan Type: *Hospital Finance*\n\n━━━━━━━━━━━━━━━━━━━━\n\n*● CUSTOMER TYPE*\n1. ${customerTypeName}\n\n*● BASIC DETAILS*\n`;

    // Build basic details based on customer type
    let fieldNum = 1;
    if (customerType === 'individual-doctor') {
      message += `${fieldNum++}. Full Name: ${formData.fullName}\n`;
      message += `${fieldNum++}. Date of Birth: ${formData.dateOfBirth}\n`;
      message += `${fieldNum++}. PAN Number: ${formData.panNumber}\n`;
      message += `${fieldNum++}. Aadhaar Number: ${formData.aadhaarNumber}\n`;
      message += `${fieldNum++}. Medical Registration Number: ${formData.medicalRegNumber}\n`;
      message += `${fieldNum++}. Specialization: ${formData.specialization}\n`;
      message += `${fieldNum++}. Years of Experience: ${formData.experience}\n`;
    } else if (['clinic-owner', 'nursing-home', 'hospital'].includes(customerType)) {
      message += `${fieldNum++}. Institution Name: ${formData.institutionName}\n`;
      message += `${fieldNum++}. Type of Facility: ${formData.facilityType}\n`;
      message += `${fieldNum++}. Registration Number: ${formData.registrationNumber}\n`;
      message += `${fieldNum++}. Year Established: ${formData.yearEstablished}\n`;
      message += `${fieldNum++}. Number of Beds: ${formData.numberOfBeds}\n`;
      message += `${fieldNum++}. Authorized Person Name: ${formData.authorizedPersonName}\n`;
      message += `${fieldNum++}. Authorized Person Designation: ${formData.authorizedPersonDesignation}\n`;
      message += `${fieldNum++}. Authorized Person PAN: ${formData.authorizedPersonPAN}\n`;
      message += `${fieldNum++}. Authorized Person Aadhaar: ${formData.authorizedPersonAadhaar}\n`;
    } else if (['partnership-firm', 'llp', 'private-limited'].includes(customerType)) {
      message += `${fieldNum++}. Company/Firm Name: ${formData.companyName}\n`;
      message += `${fieldNum++}. CIN/LLP/Partnership Reg. Number: ${formData.cinNumber}\n`;
      message += `${fieldNum++}. PAN of Company: ${formData.companyPAN}\n`;
      if (formData.gstNumber) message += `${fieldNum++}. GST Number: ${formData.gstNumber}\n`;
      message += `${fieldNum++}. Date of Incorporation: ${formData.dateOfIncorporation}\n`;
      message += `${fieldNum++}. Authorized Director/Partner Name: ${formData.authorizedPersonName}\n`;
      message += `${fieldNum++}. Director PAN: ${formData.authorizedPersonPAN}\n`;
      message += `${fieldNum++}. Director Aadhaar: ${formData.authorizedPersonAadhaar}\n`;
    } else if (customerType === 'trust-society') {
      message += `${fieldNum++}. Trust/Society Name: ${formData.trustName}\n`;
      message += `${fieldNum++}. Registration Number: ${formData.trustRegNumber}\n`;
      message += `${fieldNum++}. PAN of Trust: ${formData.trustPAN}\n`;
      message += `${fieldNum++}. Registration Date: ${formData.registrationDate}\n`;
      message += `${fieldNum++}. Authorized Trustee Name: ${formData.trusteeName}\n`;
      message += `${fieldNum++}. Trustee PAN: ${formData.trusteePAN}\n`;
      message += `${fieldNum++}. Trustee Aadhaar: ${formData.trusteeAadhaar}\n`;
    }

    // Add common address fields
    message += `${fieldNum++}. Address: ${formData.currentAddress}\n`;
    message += `${fieldNum++}. City: ${formData.city}\n`;
    message += `${fieldNum++}. PIN Code: ${formData.pinCode}\n`;

    // Add loan details
    message += `\n*● LOAN DETAILS*\n`;
    message += `1. Loan Purpose: ${formData.loanPurpose}\n`;
    message += `2. Project Location: ${formData.projectLocation}\n`;
    message += `3. Loan Amount: ₹${formatCurrency(formData.loanAmount)}\n`;
    
    const selectedTenure = tenures.find(t => t.months.toString() === formData.tenure);
    message += `4. Preferred Tenure: ${selectedTenure ? selectedTenure.label : formData.tenure + ' months'}\n`;
    message += `5. Existing Loan: ${formData.existingLoan === 'yes' ? 'Yes' : 'No'}\n`;
    
    if (formData.projectDescription) {
      message += `6. Project Description: ${formData.projectDescription}\n`;
    }
  ...
  if (submitted) {
    ...
    return <LoanSuccessCard 
      ...
      contactMessage="Our healthcare finance expert will contact you within 2 business hours (Mon-Sat: 9AM to 7PM)."
    />;
  }
