...
export function IndustryFinanceForm() {
  ...
  const buildIndustryFinanceWhatsAppMessage = (appId) => {
    ...
    message += `● Loan Type: *Industry Finance*\n\n`;
    message += `━━━━━━━━━━━━━━━━━━━━\n\n`;
    
    // Industry Type
    message += `*● INDUSTRY TYPE*\n`;
    message += `1. Industry: ${formData.industryType}\n\n`;
    
    // Business Details
    message += `*● BUSINESS DETAILS*\n`;
    let businessDetailNum = 1;
    message += `${businessDetailNum++}. Business Structure: ${formData.businessStructure}\n`;
    message += `${businessDetailNum++}. Finance Type: ${formData.financeType}\n`;
    message += `${businessDetailNum++}. Owner/Director Name: ${formData.ownerName}\n`;
    message += `${businessDetailNum++}. Date of Birth: ${formData.dob}\n`;
    message += `${businessDetailNum++}. PAN Card Number: ${formData.panNumber}\n`;
    message += `${businessDetailNum++}. Aadhaar Number: ${formData.aadhaarNumber}\n`;
    message += `${businessDetailNum++}. Current Address: ${formData.currentAddress}\n`;
    message += `${businessDetailNum++}. PIN Code: ${formData.pinCode}\n`;
    message += `${businessDetailNum++}. Business Name: ${formData.businessName}\n`;
    message += `${businessDetailNum++}. Business PAN: ${formData.businessPan}\n`;
    message += `${businessDetailNum++}. GSTIN Number: ${formData.gstin}\n`;
    message += `${businessDetailNum++}. Business Address: ${formData.businessAddress}\n`;
    message += `${businessDetailNum++}. Business Vintage: ${formData.businessVintage} years\n`;
    message += `${businessDetailNum++}. Annual Turnover: ${formatCurrency(formData.annualTurnover)}\n\n`;
    
    // Loan Details
    message += `*● LOAN DETAILS*\n`;
    let loanDetailNum = 1;
    message += `${loanDetailNum++}. Loan Amount: ${formatCurrency(formData.loanAmount)}\n`;
    message += `${loanDetailNum++}. Loan Tenure: ${formData.loanTenure} months\n`;
    message += `${loanDetailNum++}. Monthly Business Income: ${formatCurrency(formData.monthlyIncome)}\n`;
    
    // Optional: Existing EMI - only include if non-zero
    if (formData.existingEmi && parseInt(formData.existingEmi) > 0) {
      message += `${loanDetailNum++}. Existing EMI/Obligations: ${formatCurrency(formData.existingEmi)}\n`;
    }
    
    message += `${loanDetailNum++}. Collateral Available: ${formData.collateralAvailable}\n`;
  ...
  const submitApplication = () => {
    ...
    const appId = `SA-IF-${Date.now().toString().slice(-8)}`;
    setApplicationId(appId);
    setSubmitted(true);
  };

  if (submitted) {
    const whatsappUrl = buildIndustryFinanceWhatsAppMessage(applicationId);
    return <LoanSuccessCard 
      applicationId={applicationId}
      whatsappUrl={whatsappUrl}
      contactMessage="Our industry finance expert will contact you within 24 hours with the best lender options tailored to your industry and profile."
    />;
  }
