...
export function CarLoanForm() {
  ...
  const buildWhatsAppMessage = () => {
    const appId = applicationId || `SA-CL-${Math.floor(10000000 + Math.random() * 90000000)}`;
    
    let message = `*SANSKRUTI ASSOCIATES*\n*New Loan Application*\n\n━━━━━━━━━━━━━━━━━━━━\n\n● Application ID: *${appId}*\n● Loan Type: *Car Loan*\n\n━━━━━━━━━━━━━━━━━━━━\n\n*● LOAN TYPE*\n1. Car Type: ${carType === 'new' ? 'New Car' : 'Used Car'}\n\n*● PERSONAL DETAILS*\n1. Name: ${formData.fullName}\n2. Date of Birth: ${formData.dateOfBirth}\n3. PAN Card Number: ${formData.panNumber}\n4. Aadhaar Number: ${formData.aadhaarNumber}\n5. Address: ${formData.currentAddress}\n6. City: ${formData.city}\n7. PIN Code: ${formData.pinCode}`;

    // Add Employment Details section only when applicable (self-employed)
    if (employmentType === 'self-employed') {
      message += `\n\n*● EMPLOYMENT DETAILS*\n1. Business Name: ${formData.businessName}\n2. Business PAN: ${formData.businessPan}\n3. GSTIN Number: ${formData.gstinNumber}\n4. Business Vintage: ${formData.businessVintage} years\n5. Annual Turnover: ₹${formatCurrency(formData.annualTurnover)}`;
    }

    // Car & Loan Details
    message += `\n\n*● CAR & LOAN DETAILS*\n1. Car Brand: ${formData.carBrand}\n2. Car Model: ${formData.carModel}\n3. Approx. On-road Price: ₹${formatCurrency(formData.onRoadPrice)}`;
    ...
    message += `\n${fieldNum++}. Loan Tenure: ${formData.loanTenure} months`;

    // Add Used Car specific fields
    if (carType === 'used' && formData.registrationNumber) {
      message += `\n${fieldNum++}. Car Registration Number: ${formData.registrationNumber}`;
    }
    if (carType === 'used' && formData.currentValue) {
      message += `\n${fieldNum++}. Approx. Current Value: ₹${formatCurrency(formData.currentValue)}`;
    }

    // Add optional fields only if filled
    if (formData.carVariant) {
      message += `\n${fieldNum++}. Car Variant: ${formData.carVariant}`;
    }
    if (formData.existingEmi) {
      message += `\n${fieldNum++}. Existing EMI per month: ₹${formatCurrency(formData.existingEmi)}`;
    }
    if (formData.dealerName) {
      message += `\n${fieldNum++}. Dealer Name: ${formData.dealerName}`;
    }
    if (formData.dealerLocation) {
      message += `\n${fieldNum++}. Dealer Location: ${formData.dealerLocation}`;
    }
  ...
  const submitApplication = () => {
    ...
    const generatedId = `SA-CL-${Math.floor(10000000 + Math.random() * 90000000)}`;
  ...
  if (submitted) {
    ...
    return <LoanSuccessCard 
      ...
      contactMessage="Our car loan expert will contact you within 24 hours with the best offers from top banks for your profile."
    />;
  }
