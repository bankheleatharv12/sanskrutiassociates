...
export default function MachineryLoanForm() {
  ...
  const buildMachineryLoanWhatsAppMessage = () => {
    let message = `*SANSKRUTI ASSOCIATES*\n*New Loan Application*\n━━━━━━━━━━━━━━━━━━━━\n\n● Application ID: *${applicationId}*\n● Loan Type: *Machinery Loan*\n\n━━━━━━━━━━━━━━━━━━━━\n\n*● CUSTOMER TYPE*\n1. Customer Type: ${customerType}\n\n*● PERSONAL DETAILS*\n`;
    
    let lineNumber = 1;
    if (formData.fullName) {
      message += `${lineNumber}. Name: ${formData.fullName}\n`;
      lineNumber++;
    }
    if (formData.dateOfBirth) {
      message += `${lineNumber}. Date of Birth: ${formData.dateOfBirth}\n`;
      lineNumber++;
    }
    if (formData.panNumber) {
      message += `${lineNumber}. PAN Card Number: ${formData.panNumber}\n`;
      lineNumber++;
    }
    if (formData.aadhaarNumber) {
      message += `${lineNumber}. Aadhaar Number: ${formData.aadhaarNumber}\n`;
      lineNumber++;
    }
    if (formData.currentAddress) {
      message += `${lineNumber}. Address: ${formData.currentAddress}\n`;
      lineNumber++;
    }
    if (formData.city) {
    ...
    if (customerType) {
      message += `${lineNumber}. Business Type: ${customerType}\n`;
      lineNumber++;
    }
    if (formData.businessPan) {
      message += `${lineNumber}. Business PAN: ${formData.businessPan}\n`;
      lineNumber++;
    }
    if (formData.gstinNumber) {
      message += `${lineNumber}. GSTIN Number: ${formData.gstinNumber}\n`;
      lineNumber++;
    }
    if (formData.businessVintage) {
      message += `${lineNumber}. Business Vintage: ${formData.businessVintage} years\n`;
      lineNumber++;
    }
    if (formData.udyamNumber) {
      message += `${lineNumber}. Udyam/MSME Number: ${formData.udyamNumber}\n`;
      lineNumber++;
    }
    if (formData.coApplicantName) {
      message += `${lineNumber}. Co-applicant Name: ${formData.coApplicantName}\n`;
      lineNumber++;
    }
    if (formData.coApplicantPan) {
      message += `${lineNumber}. Co-applicant PAN: ${formData.coApplicantPan}\n`;
      lineNumber++;
    }
    ...
    if (formData.machineryType) {
      message += `${lineNumber}. Machinery Type: ${formData.machineryType}\n`;
      lineNumber++;
    }
    if (formData.machineryCondition) {
      message += `${lineNumber}. Machinery Condition: ${formData.machineryCondition}\n`;
      lineNumber++;
    }
    if (formData.machinerySupplierName) {
      message += `${lineNumber}. Machinery Supplier Name: ${formData.machinerySupplierName}\n`;
      lineNumber++;
    }
    if (formData.machineryCost) {
      message += `${lineNumber}. Machinery Cost: ₹${Number(formData.machineryCost).toLocaleString('en-IN')}\n`;
      lineNumber++;
    }
    if (formData.loanAmount) {
      message += `${lineNumber}. Loan Amount: ₹${Number(formData.loanAmount).toLocaleString('en-IN')}\n`;
      lineNumber++;
    }
    if (formData.loanTenure) {
      message += `${lineNumber}. Loan Tenure: ${formData.loanTenure} months\n`;
      lineNumber++;
    }
    if (formData.annualBusinessTurnover) {
      message += `${lineNumber}. Annual Business Turnover: ₹${Number(formData.annualBusinessTurnover).toLocaleString('en-IN')}\n`;
      lineNumber++;
    }
    if (formData.monthlyBusinessIncome) {
      message += `${lineNumber}. Monthly Business Income: ₹${Number(formData.monthlyBusinessIncome).toLocaleString('en-IN')}\n`;
      lineNumber++;
    }
    if (formData.existingEmi) {
      message += `${lineNumber}. Existing EMI per month: ₹${Number(formData.existingEmi).toLocaleString('en-IN')}\n`;
      lineNumber++;
    }
    if (formData.collateralAvailable) {
      message += `${lineNumber}. Collateral: ${formData.collateralAvailable}\n`;
      lineNumber++;
    }
    if (formData.purposeOfLoan) {
      message += `${lineNumber}. Loan Purpose: ${formData.purposeOfLoan}\n`;
      lineNumber++;
    }
  ...
  const submitApplication = () => {
    ...
    const genApplicationId = `SA-ML-${Date.now().toString().slice(-8)}`;
  ...
  if (submitted) {
    const whatsappUrl = buildMachineryLoanWhatsAppMessage();
    return <LoanSuccessCard 
      applicationId={applicationId}
      whatsappUrl={whatsappUrl}
      contactMessage="Our machinery loan expert will contact you within 24 hours. We will connect you with the best equipment finance lender for your profile."
    />;
  }
