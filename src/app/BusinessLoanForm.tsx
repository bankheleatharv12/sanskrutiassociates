...
import { LoanSuccessCard } from './components/LoanSuccessCard';
import {
  Building2,
  Check,
  CheckCircle2,
...
export function BusinessLoanForm() {
  ...
  const buildWhatsAppMessage = () => {
    const appId = applicationId || `SA-BL-${Math.floor(10000000 + Math.random() * 90000000)}`;
    
    let message = `*SANSKRUTI ASSOCIATES*\n*New Loan Application*\n\n━━━━━━━━━━━━━━━━━━━━\n\n● Application ID: *${appId}*\n● Loan Type: *Business Loan*\n\n━━━━━━━━━━━━━━━━━━━━\n\n*● BUSINESS TYPE*\n1. ${businessType}\n\n*● PERSONAL DETAILS*\n1. Owner Name: ${formData.ownerFullName}\n2. Date of Birth: ${formData.dateOfBirth}\n3. PAN Number: ${formData.panNumber}\n4. Aadhaar Number: ${formData.aadhaarNumber}\n5. Address: ${formData.currentAddress}\n6. City: ${formData.city}\n7. PIN Code: ${formData.pinCode}\n8. Business Name: ${formData.businessName}\n9. Business PAN Number: ${formData.businessPanNumber}\n10. GSTIN Number: ${formData.gstinNumber}\n11. Business Vintage: ${formData.businessVintage} years`;

    // Add Udyam/MSME Registration only if filled
    if (formData.udyamRegistration) {
      message += `\n12. Udyam/MSME Registration: ${formData.udyamRegistration}`;
    }

    message += `\n\n*● LOAN DETAILS*\n1. Loan Amount: ₹${formatCurrency(formData.loanAmount)}\n2. Loan Tenure: ${formData.loanTenure} months\n3. Annual Business Turnover: ₹${formatCurrency(formData.annualTurnover)}\n4. Monthly Business Income: ₹${formatCurrency(formData.monthlyBusinessIncome)}`;

    // Add optional fields only if filled
    let fieldNum = 5;
    if (formData.existingEmi) {
      message += `\n${fieldNum}. Existing EMI: ₹${formatCurrency(formData.existingEmi)}`;
      fieldNum++;
    }
    message += `\n${fieldNum}. Collateral Available: ${formData.collateralAvailable ? 'Yes' : 'No'}`;
    fieldNum++;
    message += `\n${fieldNum}. Repayment Preference: ${formData.repaymentPreference}`;
    fieldNum++;
    if (formData.loanPurpose) {
      message += `\n${fieldNum}. Loan Purpose: ${formData.loanPurpose}`;
    }
  ...
  const submitApplication = () => {
    ...
    const generatedId = `SA-BL-${Math.floor(10000000 + Math.random() * 90000000)}`;
  ...
  if (submitted) {
    ...
    return <LoanSuccessCard 
      ...
      contactMessage="Our business loan expert will contact you within 24 hours with best lender options for your profile."
    />;
  }
            ...
            <div className="[animation:loanStepIn_0.3s_ease-in-out_both]">
              ...
              <div className="space-y-5">
                ...
                <label className="flex items-start gap-3 text-gray-700">
                  ...
                  <span>I agree to the Terms & Conditions and authorize Sanskruti Associates to contact me regarding my business loan application. <span className="text-red-500">*</span></span>
                </label>
